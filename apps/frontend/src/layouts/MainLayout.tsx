...
/* ---- Navigation config (only existing routes) ---- */
...
function SidebarContent({
  ...
  const principalItems = itemsWithBadge.filter((i) => i.section === 'principal');
  const sistemaItems   = itemsWithBadge.filter((i) => i.section === 'sistema');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flex: 1, padding: '12px 10px', overflowY: 'auto', overflowX: 'hidden' }}>
        {/* Principal section */}
        {!collapsed && (
          <div className="it-sidebar-section-label">Principal</div>
        )}
        ...
        {/* Sistema section — only rendered when it has items */}
          ...
          <header className="it-header">
            ...
            {isMobile ? (
              ...
              <Tooltip title={collapsed ? 'Expandir menú' : 'Colapsar menú'} placement="right">
                <button
                  className="it-hamburger hide-mobile"
                  style={{ display: 'flex' }}
            ...
            <div className="it-search" role="search">
              ...
              <input
                placeholder="Buscar equipo, serie, código..."
                aria-label="Buscar"
              />
            </div>
            ...
            <Tooltip title={`${usuario?.nombre ?? 'Usuario'} · ${usuario?.rol ?? ''} — Cerrar sesión`}>
              <div
                ...
                onMouseEnter={(e) => {
                  (e.currentTarget as HTMLDivElement).style.boxShadow = '0 0 0 3px rgba(59,130,246,0.25)';
                }}
                onMouseLeave={(e) => {
